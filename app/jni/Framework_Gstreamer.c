...
app_function(GstreamerInstance *data) {
    ...
    /* Build pipeline */
    //GstElement *pipeline = gst_parse_launch("rtspsrc location=rtsp://192.168.1.239:8554/left latency=100 drop-on-latency=true ! application/x-rtp,encoding-name=H264 ! decodebin !  appsink emit-signals=true name=wallesink", &error);
    GstElement *pipeline = gst_parse_launch("videotestsrc pattern=snow ! video/x-raw,width=512,height=512,format=RGBA ! appsink emit-signals=true name=wallesink", &error);
