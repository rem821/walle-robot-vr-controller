...
void main()
{
    ...
    static void
    SetObjectText(OvrGuiSys &guiSys, VRMenu *menu, char const *name, char const *fmt, ...) {
        VRMenuObject *obj = menu->ObjectForName(guiSys, name);
        if (obj != nullptr) {
            char text[1024];
            va_list argPtr;
...
// ovrControllerRibbon::Update
    void ovrControllerRibbon::Update(
        ...
        UpdateRibbon(*Points, *Velocities, anchorPos, Length / Points->GetMaxPoints(),
                     deltaSeconds);
#endif
...
// ovrVrInput::AppInit
    bool ovrVrInput::AppInit(const OVRFW::ovrAppContext *context) {
        ...
        gstreamer_thread = std::thread(gstreamer_input());
...
// ovrVrInput::AppFrame
    ...
    void
    ovrVrInput::RenderRunningFrame(const OVRFW::ovrApplFrameIn &in, OVRFW::ovrRendererOutput &out) {
            ...
            if (device->GetType() == ovrControllerType_TrackedRemote) {
                ...
                if (Ribbons[trDevice.GetHand()] != nullptr) {
                    Ribbons[trDevice.GetHand()]->Update(
                            out.FrameMatrices.CenterView, ovrMatrix4f_GetTranslation(mat),
                            in.DeltaSeconds);
                }
            }
        }
        ...
        RemoteBeamRenderer->RenderEyeView(out.FrameMatrices.CenterView, projectionMatrix,
                                          out.Surfaces);
    }
    ...
    void
    ovrVrInput::AppRenderFrame(const OVRFW::ovrApplFrameIn &in, OVRFW::ovrRendererOutput &out) {
        switch (RenderState) {
    ...
    void ovrVrInput::ClearAndHideMenuItems() {
        ...
        SetObjectColor(*GuiSys, Menu, "primary_input_triggerana",
        ...
        SetObjectColor(*GuiSys, Menu, "primary_input_index_point",
                       Vector4f(0.25f, 0.25f, 0.25f, 1.0f));
        SetObjectColor(*GuiSys, Menu, "primary_input_thumb_up",
                       Vector4f(0.25f, 0.25f, 0.25f, 1.0f));

        SetObjectColor(*GuiSys, Menu, "secondary_input_trigger",
                       Vector4f(0.25f, 0.25f, 0.25f, 1.0f));
        SetObjectColor(
                *GuiSys, Menu, "secondary_input_triggerana", Vector4f(0.25f, 0.25f, 0.25f, 1.0f));
        SetObjectColor(
                *GuiSys, Menu, "secondary_input_touchpad_click",
                Vector4f(0.25f, 0.25f, 0.25f, 1.0f));
        ...
        SetObjectColor(*GuiSys, Menu, "secondary_input_gripana",
                       Vector4f(0.25f, 0.25f, 0.25f, 1.0f));
        SetObjectColor(
                *GuiSys, Menu, "secondary_input_index_point", Vector4f(0.25f, 0.25f, 0.25f, 1.0f));
        SetObjectColor(*GuiSys, Menu, "secondary_input_thumb_up",
                       Vector4f(0.25f, 0.25f, 0.25f, 1.0f));
    ...
    ovrResult ovrVrInput::PopulateRemoteControllerInfo(ovrInputDevice_TrackedRemote &trDevice) {
        ...
        if (remoteInputState.Touches & ovrTouch_IndexTrigger) {
            buttons += "TA ";
            SetObjectColor(
                    *GuiSys, Menu, triggerAnalogObjectName.c_str(),
                    Vector4f(0.25f, 1.0f, 0.0f, 1.0f));
        }

        SetObjectText(
                *GuiSys, Menu, triggerAnalogObjectName.c_str(), "%.2f",
        ...
        if (remoteInputState.Touches & ovrTouch_ThumbUp) {
            ...
            if (inputTrackedRemoteCapabilities->TouchCapabilities & ovrTouch_LThumbRest) {
                SetObjectColor(
                        *GuiSys, Menu, thumbUpObjectName.c_str(),
                        Vector4f(0.5f, 1.0f, 0.25f, 1.0f));
            }
            if (inputTrackedRemoteCapabilities->TouchCapabilities & ovrTouch_RThumbRest) {
                SetObjectColor(
                        *GuiSys, Menu, thumbUpObjectName.c_str(),
                        Vector4f(0.25f, 1.0f, 0.5f, 1.0f));
            }
        }
        ...
        if (inputTrackedRemoteCapabilities->ControllerCapabilities &
            ...
            if (remoteInputState.Buttons & ovrButton_Enter) {
                SetObjectColor(
                        *GuiSys, Menu, touchpadClickObjectName.c_str(),
                        Vector4f(1.0f, 0.25f, 0.25f, 1.0f));
            }
            ...
            if (remoteInputState.Joystick.x != remoteInputState.JoystickNoDeadZone.x ||
                ...
                SetObjectColor(
                        *GuiSys, Menu, touchPosObjectName.c_str(),
            ...
            if (controllerHand == ovrArmModel::HAND_LEFT) {
                leftJoystick = remoteInputState.Joystick.y;
            } else if (controllerHand == ovrArmModel::HAND_RIGHT) {
                rightJoystick = remoteInputState.Joystick.y;
            }
        }
    ...
    void ovrVrInput::OnDeviceConnected(const ovrInputCapabilityHeader &capsHeader) {
        ...
        switch (capsHeader.Type) {
            case ovrControllerType_TrackedRemote: {
                ...
                result =
                        vrapi_GetInputDeviceCapabilities(GetSessionObject(),
                                                         &remoteCapabilities.Header);
                if (result == ovrSuccess) {
                    device =
                            ovrInputDevice_TrackedRemote::Create(*this, *GuiSys, *Menu,
                                                                 remoteCapabilities);
            ...
            default: ALOG("Unknown device connected of type %d", capsHeader.Type);
                return;
        }
...
// ovrInputDevice_TrackedRemote::Create
    ovrInputDeviceBase *ovrInputDevice_TrackedRemote::Create(
        ...
        ovrResult result = vrapi_GetCurrentInputState(
                app.GetSessionObject(), remoteCapabilities.Header.DeviceID,
                &remoteInputState.Header);
        if (result == ovrSuccess) {
            ovrInputDevice_TrackedRemote *device = new ovrInputDevice_TrackedRemote(
                    remoteCapabilities);
...
// ovrInputDevice_TrackedRemote::Create
    void
    ovrInputDevice_TrackedRemote::UpdateHaptics(ovrMobile *ovr, const ovrApplFrameIn &vrFrame) {
                ...
                if (trigDown) {
                    if (GetTrackedRemoteCaps().ControllerCapabilities &
                        ...
                        for (int i = 0; i < maxSamples; i++) {
                            ...
                            intensity +=
                                    (float) GetTrackedRemoteCaps().HapticSampleDurationMS *
                                    0.001f;
    ...
    static void cb_message(GstBus *bus, GstMessage *msg, GStreamerData *data) {
        ...
        switch (GST_MESSAGE_TYPE (msg)) {
            case GST_MESSAGE_ERROR: {
                ...
                ALOG("WalleVrController: Error: %s\n", err->message)
                g_error_free(err);
                g_free(debug);
            ...
            case GST_MESSAGE_STATE_CHANGED:
                ...
                ALOG("WalleVrController: Element %s changed state from %s to %s.\n",
    ...
    /* Main method for the GStreamer native code. This is executed on its own thread. */
    void gstreamer_input::operator()() {
        GstStateChangeReturn ret;
        /* Initialize GStreamer */
        gst_init(nullptr, nullptr);
        ALOG("WalleVrController: gstreamer?");

        /* Build pipeline */
        gstData.pipeline = gst_parse_launch(
                "rtspsrc location=rtsp://192.168.1.239:8554/test latency=100 drop-on-latency=true ! application/x-rtp,encoding-name=H264 ! decodebin ! autovideosink sync=false",
                nullptr);
        ...
        if (ret == GST_STATE_CHANGE_FAILURE) {
            ALOG("WalleVrController: Gstreamer state change fucked");
            return;
        }

        gstData.video_sink = gst_bin_get_by_interface(GST_BIN (gstData.pipeline),
                                                      GST_TYPE_VIDEO_OVERLAY);

        if (!gstData.video_sink) {
            ALOG("WalleVrController: Could not retrieve video sink");
            return;
        }

        ALOG("WalleVrController: gstreamer??");
        gstData.main_loop = g_main_loop_new(NULL, FALSE);
        g_signal_connect (gstData.bus, "message", G_CALLBACK(cb_message), NULL);
        ALOG("WalleVrController: gstreamer???");
        g_main_loop_run(gstData.main_loop);

        /* Free resources */
        if (gstData.msg != nullptr)
        ...
        ALOG("WalleVrController: gstreamer done for good");
    }

    void ovrVrInput::sendUDPPacket() {
        if (leftJoystick == -1 || rightJoystick == -1) { return; }
        ...
        int left = (int) (leftJoystick * 1000);
        int right = (int) (rightJoystick * 1000);
        ...
        std::string message = string_format("s0:000,s1:000,m0:%04d,%01d,m1:%04d,%01d\n", abs(left),
                                            leftDir, abs(right), rightDir);

        int n_bytes = ::sendto(sock, message.c_str(), message.length(), 0,
                               reinterpret_cast<sockaddr *>(&destination), sizeof(destination));

        //ALOG("WalleVrController: send message: %s", message.c_str())
        ::close(sock);
