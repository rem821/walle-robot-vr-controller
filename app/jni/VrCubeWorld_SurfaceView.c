...
static void EglInitExtensions() {
    ...
    if (allExtensions != NULL) {
        //ALOGV("Available GlExtensions: %s", allExtensions);
    ...
    ALOGV("Extensions available: multi_view: %s, texture_border_clamp %s",
          glExtensions.multi_view ? "true" : "false",
          glExtensions.EXT_texture_border_clamp ? "true" : "false");
}
...
static void ovrEgl_CreateContext(ovrEgl *egl, const ovrEgl *shareEgl) {
    ...
    ALOGV("        eglInitialize( Display, %d, %d )", egl->MajorVersion, egl->MinorVersion);
...
static void ovrGeometry_CreateVideoPlane(ovrGeometry *geometry) {
    typedef struct {
        char positions[4][4];
...
static bool ovrProgram_Create(
    ...
    for (int i = 0;
         i < (int) (sizeof(ProgramVertexAttributes) / sizeof(ProgramVertexAttributes[0]));
         i++) {
...
static bool ovrFramebuffer_Create(
    ...
    ALOGV("Creating FrameBuffer: width %d, height %d, multiview %s, multisamples %d", width, height,
          useMultiview ? "true" : "false", multisamples);
    ...
    frameBuffer->FrameBuffers = (GLuint *) malloc(
            frameBuffer->TextureSwapChainLength * sizeof(GLuint));
    ALOGV("TextureSwapChainLength: %d", frameBuffer->TextureSwapChainLength);
...
static void ovrScene_Create(ovrScene *scene, bool useMultiview) {
    ...
    ovrGeometry_CreateVideoPlane(&scene->Cube);
    //GLuint tex = loadBMP_custom("/storage/emulated/0/Android/data/cz.walle.wallevrcontroller2/files/texture.bmp");

    // Create the instance transform attribute buffer.
    GL(glGenBuffers(1, &scene->InstanceTransformBuffer));
    GL(glBindBuffer(GL_ARRAY_BUFFER, scene->InstanceTransformBuffer));
    GL(glBufferData(GL_ARRAY_BUFFER, 4 * 4 * sizeof(float), NULL, GL_DYNAMIC_DRAW));
    GL(glBindBuffer(GL_ARRAY_BUFFER, 0));
...
ovrRenderer_Create(ovrRenderer *renderer, const ovrJava *java, const bool useMultiview) {
    ...
    ALOGV("Renderer creating %d frame buffers", renderer->NumBuffers);
...
static void ovrBmpImage_Clear(ovrBmpImage *ovrImage) {
    memset(ovrImage->header, 0, 54 * sizeof(char));
    ovrImage->dataPos = 0;
    ovrImage->width = 0;
    ovrImage->height = 0;
    ovrImage->imageSize = 0;
...
GLuint loadBMP_custom(const char *imagepath) {
    ...
    if (!file) {
        ALOGV("Image could not be opened\n");
        return 0;
    }

    ovrBmpImage image;
    ovrBmpImage_Clear(&image);

    if (fread(image.header, 1, 54, file) != 54) { // If not 54 bytes read : problem
        ALOGV("Not a correct BMP file\n");
        return false;
    }
    ...
    if (image.dataPos == 0) image.dataPos = 54; // The BMP header is done that way

    // Create a buffer
    image.data = (unsigned char *) malloc(image.imageSize * sizeof(unsigned char));
    ...
    GL(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, image.width, image.height, 0, GL_BGR,
...
void *AppThreadFunction(void *parm) {
    ...
    ALOGV("AppState UseMultiview : %s", appState.UseMultiview ? "true" : "false");
