...
static void ovrGeometry_CreatePlane(ovrVkContext* context, ovrVkGeometry* geometry) {
    // The cube is centered about the origin and spans the [-1,1] XYZ range.
    static const ovrVector3f planePositions[4] = {
            {-10.0f, 10.0f, 0.0f},
            {-10.0f, -10.0f, 0.0f},
            {10.0f, -10.0f, 0.0f},
            {10.0f, 10.0f, 0.0f}
    ...
    static const ovrVector4f planeColors[4] = {
            {0.0f, 1.0f, 0.0f, 1.0f},
            {0.0f, 1.0f, 0.0f, 1.0f},
            {0.0f, 1.0f, 0.0f, 1.0f},
            {0.0f, 1.0f, 0.0f, 1.0f}
    ...
    static const ovrTriangleIndex planeIndeces[6] = {
            0, 2, 1, 2, 0, 3
    };

    ovrDefaultVertexAttributeArrays planeAttributeArrays;
    ovrVkVertexAttributeArrays_Alloc(
            &planeAttributeArrays.base,
            DefaultVertexAttributeLayout,
            4,
            OVR_VERTEX_ATTRIBUTE_FLAG_POSITION | OVR_VERTEX_ATTRIBUTE_FLAG_COLOR);

    for (int i = 0; i < 4; i++) {
        planeAttributeArrays.position[i].x = planePositions[i].x;
        planeAttributeArrays.position[i].y = planePositions[i].y;
        planeAttributeArrays.position[i].z = planePositions[i].z;
        planeAttributeArrays.color[i].x = planeColors[i].x;
        planeAttributeArrays.color[i].y = planeColors[i].y;
        planeAttributeArrays.color[i].z = planeColors[i].z;
        planeAttributeArrays.color[i].w = planeColors[i].w;
    }

    ovrVkTriangleIndexArray planeIndexArray;
    ovrVkTriangleIndexArray_Alloc(&planeIndexArray, 6, planeIndeces);

    ovrVkGeometry_Create(context, geometry, &planeAttributeArrays.base, &planeIndexArray);

    ovrVkVertexAttributeArrays_Free(&planeAttributeArrays.base);
    ovrVkTriangleIndexArray_Free(&planeIndexArray);
}
...
static void ovrScene_Create(ovrVkContext* context, ovrScene* scene, ovrVkRenderPass* renderPass) {
    ...
    ovrGeometry_CreatePlane(context, &scene->Cube);
    ...
    for (int i = 0; i < NUM_ROTATIONS; i++) {
        scene->Rotations[i].x = 0;
        scene->Rotations[i].y = 0;
        scene->Rotations[i].z = 0;
    }
